...
def registrar_memoria(source_name: str):
    ...
    def decorator(func_ref: Callable) -> Callable:
        ...
        def wrapper(req) -> Any:  # Usar Any para evitar problemas de tipo
            logging.info(
                f"🧠 WRAPPER MEMORIA EJECUTÁNDOSE: {source_name} - {req.method} {req.url}")
            ...
            try:
                ...
                fue_redirigido, respuesta_redirigida = aplicar_deteccion_intencion(
                    req, endpoint)

                if fue_redirigido and respuesta_redirigida:
                    logging.info(
                        f"🔄 Redirección automática aplicada desde {source_name}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            registrar_redireccion_cosmos(
                                req, endpoint, fue_redirigido, respuesta_redirigida)
                        except Exception as cosmos_err:
                            logging.warning(
                                f"⚠️ Error logging Cosmos redirección: {cosmos_err}")

                    except Exception as e:
                        logging.warning(
                            f"⚠️ Error registrando redirección: {e}")
            ...
            try:
                # 🔍 DEBUG: Ver qué envía realmente Foundry
                try:
                    body_json = req.get_json() if req.get_body() else {}
                    logging.info(
                        f"📝 Body JSON recibido: {list(body_json.keys()) if body_json else 'vacío'}")
                except:
                    body_json = {}
                    logging.info("⚠️ No se pudo parsear body JSON")

                logging.info(
                    f"📝 Headers: {dict(req.headers) if hasattr(req, 'headers') else 'N/A'}")
                logging.info(
                    f"📝 Params: {dict(req.params) if hasattr(req, 'params') else 'N/A'}")

                # DETECCIÓN AUTOMÁTICA de session_id y agent_id - PRIORIZAR BODY JSON
                session_id = (
                    body_json.get("session_id")
                    or body_json.get("Session-ID")
                    or getattr(req, "headers", {}).get("Session-ID")
                ...
                agent_id = (
                    ...
                    or body_json.get("Agent-ID")
                    or getattr(req, "headers", {}).get("Agent-ID")
                ...
                if session_id.startswith("auto_"):
                    logging.warning(
                        f"⚠️ Session ID no encontrado en headers ni params, generado fallback: {session_id}")
                else:
                    logging.info(f"✅ Session ID preservado: {session_id}")

                logging.info(
                    f"🔍 IDs detectados - Session: {session_id}, Agent: {agent_id}")

                # 🔥 CONSULTAR MEMORIA CRONOLÓGICA SIEMPRE (GET o POST)
                if session_id and agent_id and not session_id.startswith("auto_"):
                    ...
                    resultado_memoria = consultar_memoria_sesion(
                        session_id, agent_id)
                    if resultado_memoria.get("exito"):
                        memoria_contexto = resultado_memoria["memoria"]
                        logging.info(
                            f"🧠 Memoria cronológica: {memoria_contexto.get('total_interacciones_sesion', 0)} interacciones")
                    else:
                        logging.info(
                            f"🆕 Nueva sesión detectada: {session_id[:8]}.../{agent_id[:10]}...")
                else:
                    logging.info(
                        f"⚠️ Sin session_id válido, memoria no consultada")
                ...
                try:
                    ...
                    logging.info(
                        f"🧠 Contexto semántico enriquecido: {len(contexto_semantico)} fuentes")

                except Exception as e:
                    logging.warning(
                        f"⚠️ Error obteniendo contexto semántico: {e}")
                ...
                if hasattr(req, '__dict__'):
                    ...
                    if memoria_contexto:
                        ...
                        req.__dict__["_memoria_prompt"] = generar_contexto_prompt(
                            memoria_contexto)
                ...
                try:
                    if hasattr(response, 'get_body'):
                        ...
                        if body_bytes:
                            ...
                            # 🔥 ENRIQUECIMIENTO AUTOMÁTICO CON MEMORIA
                            if memoria_contexto or contexto_semantico:
                                enriquecimiento = {}

                                # Agregar contexto previo
                                if memoria_contexto and isinstance(memoria_contexto, dict):
                                    resumen = memoria_contexto.get(
                                        'resumen_ultimo') or memoria_contexto.get('ultimo_tema')
                                    if resumen:
                                        enriquecimiento["contexto_previo"] = str(resumen)[
                                            :300]
                                    total = memoria_contexto.get(
                                        'total_interacciones_sesion', 0)
                                    if total > 0:
                                        enriquecimiento["interacciones_previas"] = total

                                # Agregar estado del sistema
                                if contexto_semantico and not contexto_semantico.get("error"):
                                    enriquecimiento["estado_sistema"] = f"{len(contexto_semantico)} fuentes activas"

                                # Enriquecer mensaje si existe
                                if "mensaje" in response_data and enriquecimiento.get("contexto_previo"):
                                    mensaje_original = response_data["mensaje"]
                                    contexto_texto = enriquecimiento["contexto_previo"]
                                    response_data[
                                        "mensaje_enriquecido"] = f"{mensaje_original} (Contexto: {contexto_texto})"

                                # Marcar memoria aplicada
                                if enriquecimiento:
                                    response_data["memoria_aplicada"] = True
                                    response_data["enriquecimiento"] = enriquecimiento
                                    logging.info(
                                        f"✅ Memoria aplicada automáticamente: {len(enriquecimiento)} elementos")

                                    # Recrear response con datos enriquecidos
                    ...
                    response_data = {"status_code": getattr(
            ...
            try:
                ...
                try:
                    ...
                    registrar_snapshot_semantico(
                        ...
                        metadata={"endpoint": source_name,
                                  "wrapper": "automatico"}
                    )

                    logging.debug(
                        f"🧠 Memoria semántica registrada automáticamente")

                except Exception as e:
                    logging.warning(
                        f"⚠️ Error registrando memoria semántica: {e}")

                logging.debug(
                    f"🧠 Memoria registrada: {source_name} ({duration_ms:.1f}ms)")
...
def crear_wrapper_memoria(app: Any):
    ...
    """
    logging.warning(
        "⚠️ crear_wrapper_memoria está deprecated. Usar memory_route_wrapper.py")
