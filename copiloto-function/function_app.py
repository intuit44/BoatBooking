...
def _error(code, status, cause, hint=None, next_steps=None, trace=None):
    return _json({
...
def invocar(req: func.HttpRequest) -> func.HttpResponse:
    """
    Resuelve y ejecuta endpoints dinámicamente usando un resolver genérico
    """
    body = _json_body(req)
    ...
    try:
        payload = json.dumps(data, ensure_ascii=False).encode(
            "utf-8") if method in {"POST", "PUT", "PATCH"} else b""
        target_req = func.HttpRequest(method=method, url=f"http://localhost{path}",
                        ...
                        for ts in item.timeseries:
                            if ts.data:
                             # Obtener el último valor
                                ultimo_valor = ts.data[-1] if ts.data else None
                                if ultimo_valor:
