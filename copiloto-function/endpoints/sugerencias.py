...
def sugerencias_http(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        for item in resultados:
            ...
            # Guard para texto_semantico faltante
            texto = item.get("texto_semantico", "") or item.get(
                "content", "") or item.get("message", "") or str(item.get("id", ""))
            if "copiloto" in texto.lower():
                acciones_frecuentes["copiloto"] = acciones_frecuentes.get(
                    "copiloto", 0) + 1
            if "diagnostico" in texto.lower():
                acciones_frecuentes["diagnostico"] = acciones_frecuentes.get(
                    "diagnostico", 0) + 1
        ...
        todos_endpoints = {"/api/copiloto",
                           "/api/diagnostico-recursos", "/api/leer-archivo"}
        ...
        top_sugerencias = sorted(
            sugerencias, key=lambda x: x["relevancia"], reverse=True)[:5]
