...
Write-Host "ðŸ§ª TEST DE LOGIC APP Y COPILOTO" -ForegroundColor Cyan
Write-Host "================================" -ForegroundColor Cyan

# URL completa con todos los parÃ¡metros
...
# Test 1: Ping simple
Write-Host "`nðŸ“ Test 1: Ping simple" -ForegroundColor Yellow
$body1 = @{
  agent_response = "ping"
...
try {
  ...
  Write-Host "âŒ Error en ping: $_" -ForegroundColor Red
  Write-Host "Status Code: $($_.Exception.Response.StatusCode.value__)" -ForegroundColor Red
}

# Test 2: Dashboard
Write-Host "`nðŸ“ Test 2: Dashboard" -ForegroundColor Yellow
$body2 = @{
...
try {
  ...
  Write-Host "âœ… Dashboard ejecutado" -ForegroundColor Green
  $response2 | ConvertTo-Json -Depth 3
}
catch {
  Write-Host "âŒ Error en dashboard: $_" -ForegroundColor Red
}

# Test 3: Verificar en App Insights
Write-Host "`nðŸ“ Test 3: Verificando en App Insights..." -ForegroundColor Yellow
...
if ($insights.tables[0].rows.Count -gt 0) {
  Write-Host "âœ… Solicitudes encontradas en App Insights:" -ForegroundColor Green
  foreach ($row in $insights.tables[0].rows) {
    Write-Host "  - $($row[0]): HTTP $($row[1]) - UA: $($row[3])" -ForegroundColor Gray
  }
}
else {
  Write-Host "âšï¸ No se encontraron solicitudes recientes" -ForegroundColor Yellow
}

Write-Host "`nâœ… TEST COMPLETADO" -ForegroundColor Green
Write-Host "Si ves respuestas 200, la Logic App estÃ¡ funcionando correctamente." -ForegroundColor Cyan
Write-Host "Si ves errores 400, revisa que la URL incluya todos los parÃ¡metros." -ForegroundColor Yellow