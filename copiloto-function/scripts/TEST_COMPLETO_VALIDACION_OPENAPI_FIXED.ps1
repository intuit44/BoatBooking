...
# Script de validación completa que simula comportamiento del agente AI
  ...
  try {
    ...
    try {
      if (!(Get-Module -ListAvailable -Name powershell-yaml)) {
        Write-Warn "Instalando módulo powershell-yaml..."
...
function New-TestScenarios {
  ...
  $validScenario = @{
    ...
    Description    = "Llamada con parámetros mínimos requeridos"
  ...
  if ($Operation.parameters -or $Operation.requestBody) {
    $invalidScenario = @{
      ...
      Description    = "Llamada sin parámetros requeridos"
...
# ============ CASOS ESPECIALES PARA PATHS ESPECÍFICOS ============
...
# ============ PREPARACIÓN DE ARCHIVOS PARA PRUEBAS ============
