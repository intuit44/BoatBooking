...
if(-not $unicaKey){ warn "No pude leer host default key. Saltando reemplazo." }
  ...
  if($hits.Count -gt 0){
    if($Apply){
      $payload = "{""properties"":{""definition"": " + ($def | ConvertTo-Json -Depth 100) + "}}"
      # usa body desde archivo para evitar error de â€œlÃ­nea demasiado largaâ€
