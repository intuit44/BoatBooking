...
from ._generated.models import StorageServiceProperties, KeyInfo, CorsRule as _GenCorsRule
...
class BlobServiceClient(StorageAccountHostsMixin, StorageEncryptionMixin):
    ...
    def __init__(
        ...
        credential: Optional[Union[str, Dict[str, str], "AzureNamedKeyCredential",
                                   "AzureSasCredential", "TokenCredential"]] = None,  # pylint: disable=line-too-long
        **kwargs: Any
    ) -> None:
        try:
        ...
        super(BlobServiceClient, self).__init__(parsed_url,
                                                service='blob', credential=credential, **kwargs)
    ...
    def get_user_delegation_key(self, key_start_time,  # type: datetime
        ...
        """
        key_info = KeyInfo(start=_to_utc_datetime(
            key_start_time), expiry=_to_utc_datetime(key_expiry_time))
    ...
    def get_account_information(self, **kwargs):
        ...
        """
        try:
            account_info = self._client.service.get_account_info(
                cls=return_response_headers, **kwargs)
            if account_info is None:
                # Ensure a dict is returned to satisfy the declared return type
                return {}
            return account_info
        except HttpResponseError as error:
            process_storage_error(error)
    ...
    def set_service_properties(
        ...
        **kwargs
        ...
        # Cast the public SDK CorsRule list to the generated model CorsRule list for the underlying model.
        # At runtime this is a no-op; this satisfies static type checkers which expect the generated model type.
        cors_param = None
        if cors is not None:
            cors_param = cast(List[_GenCorsRule], cors)
    ...
    def _rename_container(self, name, new_name, **kwargs):
        ...
        try:
            renamed_container._client.container.rename(
                name, **kwargs)   # pylint: disable = protected-access
    ...
    def undelete_container(self, deleted_container_name, deleted_container_version, **kwargs):
        ...
        if new_name:
            warnings.warn("`new_name` is no longer supported.",
                          DeprecationWarning)
        container = self.get_container_client(
            new_name or deleted_container_name)
        try:
    ...
    def get_container_client(self, container):
        ...
        """
        # determine container_name safely
        if isinstance(container, str):
            container_name = container
        else:
            # use getattr to avoid static analyzers complaining about accessing `.name` on str
            container_name = getattr(container, 'name', container)

        # Validate that container_name is a string before passing to ContainerClient
        if not isinstance(container_name, str):
            raise TypeError(
                "container must be a str or an object with a 'name' attribute that is a str")
        ...
        return ContainerClient(
            ...
            credential=cast(Optional[Union[str, Dict[str, str], "AzureNamedKeyCredential", "AzureSasCredential", "TokenCredential"]], self.credential), api_version=self.api_version, _configuration=self._config,
        ...
        *,
        ...
        if isinstance(container, str):
            ...
            try:
                ...
                container_name = container
        # Validate that container_name is a string before proceeding
        if not isinstance(container_name, str):
            raise TypeError(
                "container must be a str or an object with a 'name' attribute that is a str")

        # determine blob_name safely
        if isinstance(blob, str):
            ...
            try:
                ...
                blob_name = blob
        # Validate that blob_name is a string before proceeding
        if not isinstance(blob_name, str):
            raise TypeError(
                "blob must be a str or an object with a 'name' attribute that is a str")
